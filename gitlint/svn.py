...
def repository_root():
    ...
    try:
        root = subprocess.check_output(['svn', 'info']) \
            .decode('utf-8').split(os.linesep)

        root = utils.filter_lines(
            root,
            r'Working Copy Root Path: (?P<root>.*)$',
            groups=('root',))
        for r in root:
            return r
...
def _remove_filename_quotes(filename):
    """Removes the quotes from a filename returned by svn status."""
...
def modified_files(root, tracked_only=False, commit=None):
    ...
    status_lines = subprocess.check_output([
        'svn', 'status', root]).decode('utf-8').split(os.linesep)
...
def _modified_files_with_commit(root, commit):
    ...
    return dict((os.path.join(root, _remove_filename_quotes(filename.strip())),
...
def modified_lines(filename, extra_data, commit=None):
    ...
    Returns: a list of lines that were modified, or None in case all lines are
      new.
    """
    return None #TODO: implement for svn!
